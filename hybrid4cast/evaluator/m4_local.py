...
class M4EvaluatorLocal(M4Evaluator):
    ...
    def run_hybrid_model(self, m4_type='Hourly', sample_hislen=None, base_season_policy='byets', long_season_policy=None,
                         ...
                         bootstrap_sampler='median', absfp_etsfit=None, pipeline_chkpt=None, error_file=True, verbose=1):

        # load fitted ets models from disk
        print_section_msg('loading fitted ets models from pickle file')
        if absfp_etsfit is None:
            absfp_etsfit = self.absfp_etsfit(m4_type)
        with open(absfp_etsfit, 'rb') as fid:
            etsfit_list = pickle.load(fid)

        nn_time_cost = time.time()
        ...
        hybrid_pipeline = HybridModelPipelineLocal(
            sample_horizon, sample_hislen, season_period, self.use_transform, True,
            base_season_policy, long_season_threshold, long_season_policy
        )
        # hybrid_pipeline.dummy_hybrid = True  # just for experiment
        self.hybrid_pipeline = hybrid_pipeline

        if pipeline_chkpt is None:
            ...
            print_section_msg('load a checkpointed pipeline')
            print('loading normalier and nn model from pipeline {}'.format(pipeline_chkpt))
            hybrid_pipeline.deserialize(pipeline_chkpt)

        print_section_msg('generating test samples')
        id_set, x_forecast_set, y_ets_normalizer_set, value_transformer_set = \
            hybrid_pipeline.get_forecast_input(etsfit_list, 0, verbose=verbose)
        ...
        nn_time_cost = time.time() - nn_time_cost
        print('cnn train/forecast time it took {0}'.format(nn_time_cost))
        ...
        id_set_set = set(id_set)
        existence_idx = [i for i, id in enumerate(id_actual) if id in id_set_set]
        id_actual = id_actual[existence_idx]
        test_actual = test_actual[existence_idx]

        smape_table = hybrid_pipeline.compute_smape(
            y_prediction, test_actual, 1, id_prediction=id_set, id_actual=id_actual
        )
        ...
        return smape_table, nn_time_cost

    def run_batch_optimal_hybrid(self, m4_type, iteration=10, **kwargs):
        if 'error_file' in kwargs.keys():
            error_file = kwargs['error_file']
            kwargs.pop('error_file')
        else:
            output_folder = os.path.join(self.hybrid_output_folder, m4_type)
            if not os.path.exists(output_folder):
                os.makedirs(output_folder)
            error_file = os.path.join(output_folder, 'smape.csv')

        smape_list, time_list = [], []
        for i in range(iteration):
            print_section_msg('iteration {}'.format(i))
            this_error_file = error_file[:-4] + '{}.csv'.format(i)
            one_smape, cnn_tc = self.run_optimal_hybrid(m4_type, verbose=0, error_file=this_error_file, **kwargs)
            smape_list.append(np.mean(one_smape.smape0.values))
            time_list.append(cnn_tc)

        batch_result = pd.DataFrame(np.array([smape_list, time_list]).T, columns=['smape', 'time'])
        print_section_msg('{} iterations statz'.format(iteration))
        print('mean smape {}, sd {}'.format(np.mean(smape_list), np.std(smape_list)))
        print('time cost average {}, sd {}\n\n'.format(np.mean(time_list), np.std(time_list)))
    ...
    def run_optimal_hybrid(self, m4_type, **kwargs):
        ...
        para_dict = CNNOptimizer.get_optimal_params(res_gp, **kwargs)
    ...
    def tune_hyper_parameters(self, m4_type='Yearly', n_calls=50, **kwargs):
        ...
        if 'absfp_etsfit' in kwargs.keys():
            absfp_etsfit = kwargs['absfp_etsfit']
            kwargs.pop('absfp_etsfit')
        else:
            absfp_etsfit = self.absfp_etsfit(m4_type)
        print_section_msg('loading fitted ets models of from pickle file {}'.format(absfp_etsfit))
        with open(absfp_etsfit, 'rb') as fid:
            etsfit_list = pickle.load(fid)
        print('{} ts loaded successfully!'.format(len(etsfit_list)))

        if 'para_space' in kwargs.keys():
            hyper_para_space = kwargs['para_space']
            kwargs.pop('para_space')
        else:
        ...
        hybrid_pipeline = HybridModelPipelineLocal(
            sample_horizon, None, season_period, 'log', True,
        ...
        if 'absfp_tune' in kwargs.keys():
            output_file = os.path.join(self.hybrid_output_folder, m4_type)
            if not os.path.exists(output_file):
                os.makedirs(output_file)
            output_file = os.path.join(output_file, kwargs['absfp_tune'])
        else:
        ...
        print('saving fitting history to {}'.format(output_file))
