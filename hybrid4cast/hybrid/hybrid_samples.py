...
class ETSNNSampleGenerator:
    ...
    def create_train_val_set(self, holdout_length=0, train_stride=None, num_val=1, num_train=None, train_random=True):
        ...
        sample_count, stride_value = 0, train_stride
        stride_increment_frequency, stride_increment_factor, stride_max_factor = 8, 1, 4
        ...
        while sample_pivot_index >= self.sample_hislen:
            ...
            if train_random:
                current_stride = np.random.randint(int(stride_value / 2), int(stride_value * 3 / 2))
            sample_pivot_index -= current_stride

            # adaptively change the stride length based on the number of samples
            sample_count += 1
            if sample_count % stride_increment_frequency == 0:
                stride_value += train_stride * stride_increment_factor
                stride_value = np.min([stride_value, stride_max_factor * train_stride])
