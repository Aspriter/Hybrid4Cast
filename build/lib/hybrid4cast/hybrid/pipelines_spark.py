...
class HybridModelPipelineSpark(HybridPipeline):
    ...
    def mapper_get_hybrid_training_samples(self, ets_data, holdout_length=0, train_stride=None, num_val=1,
    ...
    def generate_training_samples(self, etsfit_rdd, holdout_length=0, train_stride=None, num_val=1,
        ...
        func_get_train_sample = self.mapper_get_hybrid_training_samples
        etsfit_rdd = etsfit_rdd.map(lambda x: func_get_train_sample(x, holdout_length, num_train, num_val, train_stride, train_random))

        # execute the transform
        sample_list = etsfit_rdd.collect()
    ...
    '''
    def mapper_get_hybrid_forecast_input(self, ets_data, holdout_length=0):
    ...
    def get_forecast_input(self, etsfit_rdd, holdout_length=0):
        # collect samples per ts from all the time series
        func_get_input_one_ts = self.mapper_get_hybrid_forecast_input
        input_list = etsfit_rdd.map(lambda x: func_get_input_one_ts(x, holdout_length)).collect()
